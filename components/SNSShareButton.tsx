...
const SNSShareButton: React.FC<SNSShareButtonProps> = ({ food }) => {
  ...
  const handleSaveImage = async () => {
    // 사용자가 요청한 디자인 요소(로고, 하단 60px 여백 등)가 포함된 레이아웃을 캡처합니다.
    const element = document.getElementById('wineat-capture-target');
    if (!element) return;

    setIsGenerating(true);
    try {
      // 폰트가 완전히 로드된 후 캡처를 진행하여 글자 깨짐 방지
      await document.fonts.ready;
      
      const dataUrl = await toPng(element, {
        backgroundColor: '#ffffff',
        cacheBust: true,
        pixelRatio: 3, // 고해상도(인쇄/SNS용) 저장을 위해 배율 상향
        skipFonts: false,
      });
