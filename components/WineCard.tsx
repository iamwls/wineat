...
const WineCard: React.FC<WineCardProps> = ({ wine, rank, isFavorite, onToggleFavorite }) => {
  ...
  return (
    <div className="bg-white rounded-3xl p-7 md:p-10 border border-stone-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
      {rank && (
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-8">
        <div className="flex-grow">
          <div className="flex flex-wrap items-center gap-4 mb-4">
            <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.15em] ${getTypeColor(wine.type)}`}>
              {wine.type}
            </span>
            <span className="text-[13px] font-bold text-stone-900">{wine.priceRange}</span>
            <div className="ml-auto flex items-center gap-1.5 bg-stone-50 px-3 py-1 rounded-full">
        ...
        <div className="flex flex-row md:flex-col items-center justify-between gap-4 md:min-w-[120px]">
          <button 
            ...
            className={`p-4 rounded-full border transition-all duration-300 ${
              isFavorite 
              ? 'bg-stone-950 border-stone-950 text-white shadow-lg shadow-stone-950/20' 
              : 'border-stone-200 text-stone-300 hover:border-stone-950 hover:text-stone-950'
            }`}
          >
          ...
          <button className="flex-grow md:flex-none px-6 py-3 bg-stone-50 text-stone-950 text-[11px] font-black uppercase tracking-widest rounded-xl hover:bg-stone-950 hover:text-white transition-all duration-300">
            VIEW DETAIL
          </button>
        </div>
      </div>
